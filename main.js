...
*/
function UpdateValue(item, type) {
  let i = item.id
  let val = item.value
  ...
  CurrentPreset.UpdateRow(target, i, val)
  UpdateTable(CurrentPreset)
  console.log(target)
  console.log(item.id)
...
function UpdateTable(preset_object) {
  ...
  for (const [key, value] of Object.entries(preset_object)) {
    ...
    <input
      ...
      type="text"
      disabled
      onchange="UpdateValue(this, 'preset-name')"
      name="${index}"
      id="name${index}"
      value="${key}"
    />
    ...
    <td>
      <input onChange="UpdateValue(this, 'databox')" class="data-box form-control" type="text" name="${index}" id="ch${index}" value="${preset_object[`${key}`].channel}" />
    </td>
    <td>
      <input onChange="UpdateValue(this, 'databox')" ${PC ? "disabled" : ""} class="data-box form-control" type="text" name="${index}" id="vel${index}" value="${preset_object[`${key}`].velocity ? preset_object[`${key}`].velocity : 0}" />
    </td>
    <td>
      <input onChange="UpdateValue(this, 'databox')" class="data-box form-control" type="text" name="${index}" id="val${index}" value="${preset_object[`${key}`].value}" />
    </td>`
    console.log(newRow)
